...
export function TimelineRuler({ duration, currentTime, zoom, onTimeChange }: TimelineRulerProps) {
  ...
  return (
    <div
      ref={rulerRef}
      className="h-8 bg-muted border-b border-border cursor-pointer relative"
      onClick={handleRulerClick}
    >
      {/* Time markers */}
      {markers.map((marker, index) => (
        <div key={index} className="absolute top-0 flex flex-col items-center" style={{ left: `${marker.position}px` }}>
          {/* Marker line */}
          <div className={`border-l ${marker.isMajor ? "border-border h-6" : "border-muted-foreground/50 h-3"}`} />

          {/* Time label */}
          {marker.label && <span className="text-xs text-muted-foreground mt-1 whitespace-nowrap">{marker.label}</span>}
        </div>
      ))}
